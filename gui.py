        ...
        if self.row_name == hardware_asset_names[0]:
            ...
            # Updating a Row Commands
                ...
                # Displays Renewal date fields for new value input
                    ...
                    self.renewal_month_inp = ttk.Combobox(self.date_frame, width= 9, values = list(month_options.keys()), state='readonly')
                ...
                tk.Button(self.window, text = f"Update {self.table_row_inp.get()}", command=self.updating_record).pack()
    ...
    def updating_record(self):
        ...
        if self.column_inp.get() == "ipAddress":
            if asset_ip_address_var == "": # Presence Check
                ...
                return
            elif len(asset_ip_address_var) < 8: # Length Check
        ...
        # Asset Renewal Date Validation
            ...
            asset_renewal_date = f"{self.renewal_day_inp.get()}/{month_options[self.renewal_month_inp.get()]}/{self.renewal_year_inp.get()}"
            # Asset Renewal Date Validation
...
class DatabaseGUI:
    def __init__(self, window, database_type):
        self.window = window
